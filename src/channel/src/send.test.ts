...
describe("AgentMail reply-only outbound", () => {
  ...
  it("reconciles an unknown send with the same queue-derived idempotency key", async () => {
    ...
    // The unknown-send reconciliation context carries no local media-access handles in this SDK, so
    // outbound attachments are recovered from their URLs without a mediaAccess passthrough.
    expect(loadAgentMailOutboundAttachments).toHaveBeenCalledWith(
  ...
  it("retries a recent triggering-message 404 as a provider projection race", async () => {
    reply.mockClear();
    const deletedClient = () =>
      ({
        inboxes: {
